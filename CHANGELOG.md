...
### 🐛 Bug修复
- **应用完整配置报错修复**
  - 修复 `ComboBox.insertSeparator` 在当前控件实现下不可用导致的异常
  - 使用禁用分隔项替代，并在分组选择逻辑中忽略分隔项
- **Skill 市场安装失败修复**
  - 为打包配置补充 `requests` 与依赖的 hidden imports
  - 修复打包版安装 Skill 报错：`No module named 'requests'`
- **MCP 对话框高度异常修复**
  - 修复“附加信息”折叠后窗口高度不回缩问题
  - 支持纵向缩放，避免内容展开后超出屏幕难以操作
- **GLM/原生 Provider 余额查询闪退修复**
  - 增加余额查询支持能力判断
  - 不支持查询时弹出提示，不再触发崩溃退出
- **全局异常处理增强**
  - 新增跨平台全局异常捕获（主线程 + 子线程）
...
- **完整的Agent组合管理**:
  ...
  - 一键应用预设分组到Agent配置页面
  - 支持自定义分组的创建、编辑、删除
  - 分组配置保存到 `~/.config/opencode/agent-groups.json`
- **6个预设分组模板**:
  - **Minimal** (最小配置): 1个OpenCode agent + 1个OMO agent
  - **Standard** (标准配置): 2个OpenCode agents + 2个OMO agents
  - **Common** (常用配置): 4个OpenCode agents + 5个OMO agents
  - **Complete** (完整配置): 7个OpenCode agents + 7个OMO agents
  - **Frontend** (前端开发): 3个OpenCode agents + 4个OMO agents
  - **Backend** (后端开发): 4个OpenCode agents + 3个OMO agents
- **Pivot标签页UI**:
  - 使用Pivot组件切换OpenCode和Oh My OpenCode agents
  - 清晰的标签页分隔线
  - 固定高度40px的标签栏
- **动态Agent列表**:
  - 从实际配置文件动态加载agent列表
  ...
  - 支持自定义agent的显示
- **Agent计数显示**:
  - 显示已选择/总数的agent计数（如 "OpenCode: 4/7  Oh My OpenCode: 5/7"）
  - Tooltip显示所有已选择的agent名称
  - 实时更新计数信息
- **灵活的Agent选择**:
  - 移除必选agent限制（之前build/sisyphus-junior为必选）
  - 所有agents均可自由选择
  - 支持创建空分组

### 🐛 Bug修复
#### **对话框布局优化** ⭐
- **问题**: Agent分组编辑对话框布局不合理，表格显示空间不足
- **修复**:
  - 减少对话框内边距和间距（20px→15px, 15px→10px）
  - 减少描述框高度（60px→50px）
  - 移除基本信息区域的滚动容器
  - 表格使用stretch factor=1占据剩余空间
  - 节省约25-30px垂直空间用于表格显示
- **文件**: `opencode_config_manager_fluent.py` (修改布局代码)

#### **OMO描述显示修复** ⭐
- **问题**: Oh My OpenCode agents描述为空时显示空白
- **修复**:
  - 添加fallback到PRESET_AGENTS字典
  - 当配置中缺少描述时，使用预设描述
  - 确保所有OMO agents都有可读的描述
- **文件**: `opencode_config_manager_fluent.py` (修改描述加载逻辑)

#### **对话框继承修复**
- **问题**: AgentGroupDialog和AgentGroupEditDialog继承自MessageBoxBase导致布局问题
- **修复**: 改为继承自QDialog，使用标准对话框布局
- **文件**: `opencode_config_manager_fluent.py` (修改类继承)

#### **OMO表格简化**
- **优化**: 移除provider和model列，仅保留enabled、agent_id、description列
- **原因**: OMO agents的provider/model信息在OMO配置页面管理，无需在分组中显示
- **文件**: `opencode_config_manager_fluent.py` (修改表格列定义)

#### **翻译文件JSON语法错误**
- **问题**: zh_CN.json中存在JSON语法错误
- **修复**: 修复JSON格式错误，确保文件可正常解析
...
- **优化**: 
  ...
  - 添加水平分隔线（1px灰色线）
  - 改善标签页视觉区分度
- **文件**: `opencode_config_manager_fluent.py` (添加样式代码)

#### **Agent配置页面集成**
- **新增**: 在OpenCode Agent和Oh My Agent配置页面添加分组选择器
- **功能**: 
  - 下拉选择已保存的分组
  - 一键应用分组配置
  - 实时更新agent启用状态
- **文件**: `opencode_config_manager_fluent.py` (新增AgentGroupWidget)

### 📚 技术实现
- **新增类**:
  - `AgentGroupManager`: 分组CRUD管理器
  - `AgentGroupDialog`: 分组选择对话框
  - `AgentGroupEditDialog`: 分组编辑对话框
  - `AgentGroupWidget`: 分组选择器组件
- **核心方法**:
  - `AgentGroupManager.load_groups()`: 加载所有分组
  - `AgentGroupManager.save_group()`: 保存分组
  - `AgentGroupManager.delete_group()`: 删除分组
  - `AgentGroupManager.get_preset_groups()`: 获取预设分组
- **配置文件**: `~/.config/opencode/agent-groups.json`

### 📁 文件变更
- 更新：`opencode_config_manager_fluent.py` (新增Agent分组管理功能)
- 更新：`locales/zh_CN.json` (新增分组相关翻译)
- 更新：`locales/en_US.json` (新增分组相关翻译)
